# URL anatomy explorer

ID: FOUNDATIONS-06. Status: specification ready; solution not supplied.
...
Observe requests and identify responsibility across browser, server and database. Apply this by building url anatomy explorer and explaining the acceptance example.

## Problem Statement

Build url anatomy explorer for a fictional college learning environment. The deliverable must demonstrate the specific behavior below using small reproducible fixtures. Use fictional records only.

## Features

- Parse five sample URLs with the URL API.
- print protocol, host, pathname and query values.

## Functional Requirements

1. FR-1: Parse five sample URLs with the URL API. Provide an observation or test that proves this behavior.
2. FR-2: print protocol, host, pathname and query values. Provide an observation or test that proves this behavior.
...
Prepare the fixture or interaction in this acceptance scenario: https://example.test/courses?page=2 yields page 2.

## Expected Output

Acceptance oracle: https://example.test/courses?page=2 yields page 2. Record the actual result next to the expected result in your test evidence.
...
Add one contrasting fixture that stresses the main invariant: parse five sample urls with the url api. Compare the two results in a short design note. For Industry projects, include operational limits and recovery steps in the handoff.

## Interview Questions

1. How does parse five sample urls with the url api work in your solution?
