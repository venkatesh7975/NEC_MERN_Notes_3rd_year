...
ID: FOUNDATIONS-03. Status: specification ready; solution not supplied.
...
Observe requests and identify responsibility across browser, server and database. Apply this by building response comparison lab and explaining the acceptance example.

## Problem Statement

Build response comparison lab for a fictional college learning environment. The deliverable must demonstrate the specific behavior below using small reproducible fixtures. Use fictional records only.

## Features

- Serve one fixed JSON file and one generated timestamp response.
- record two reads of each.

## Functional Requirements

1. FR-1: Serve one fixed JSON file and one generated timestamp response. Provide an observation or test that proves this behavior.
2. FR-2: record two reads of each. Provide an observation or test that proves this behavior.
...
Prepare the fixture or interaction in this acceptance scenario: Fixed file stays identical; generated timestamp changes.

## Expected Output

Acceptance oracle: Fixed file stays identical; generated timestamp changes. Record the actual result next to the expected result in your test evidence.
...
Add one contrasting fixture that stresses the main invariant: serve one fixed json file and one generated timestamp response. Compare the two results in a short design note. For Industry projects, include operational limits and recovery steps in the handoff.

## Interview Questions

1. How does serve one fixed json file and one generated timestamp response work in your solution?
