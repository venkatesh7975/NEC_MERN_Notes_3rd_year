...
ID: FOUNDATIONS-01. Status: specification ready; solution not supplied.
...
Observe requests and identify responsibility across browser, server and database. Apply this by building request diary and explaining the acceptance example.

## Problem Statement

Build request diary for a fictional college learning environment. The deliverable must demonstrate the specific behavior below using small reproducible fixtures. Use fictional records only.

## Features

- Record five local page requests with method, path, status and content type.
- distinguish a cached response from a fresh one.

## Functional Requirements

1. FR-1: Record five local page requests with method, path, status and content type. Provide an observation or test that proves this behavior.
2. FR-2: distinguish a cached response from a fresh one. Provide an observation or test that proves this behavior.
...
Prepare the fixture or interaction in this acceptance scenario: Reload index.html; record document and stylesheet requests.

## Expected Output

Acceptance oracle: Reload index.html; record document and stylesheet requests. Record the actual result next to the expected result in your test evidence.
...
Add one contrasting fixture that stresses the main invariant: record five local page requests with method, path, status and content type. Compare the two results in a short design note. For Industry projects, include operational limits and recovery steps in the handoff.

## Interview Questions

1. How does record five local page requests with method, path, status and content type work in your solution?
