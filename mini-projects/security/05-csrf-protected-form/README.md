...
ID: SECURITY-05. Status: specification ready; solution not supplied.
...
Apply owasp basics, sql injection, xss, csrf to implement csrf-protected form. Explain the acceptance example from input to result.

## Problem Statement

Build csrf-protected form for a fictional college environment. Validate a server-issued token and origin for cookie-authenticated mutations.

## Features

- Validate a server-issued token and origin for cookie-authenticated mutations.

## Functional Requirements

1. FR-1: Validate a server-issued token and origin for cookie-authenticated mutations. Demonstrate with a reproducible fixture.
...
Prepare the fixture and interaction needed for this scenario: Missing token rejects mutation.

## Expected Output

Missing token rejects mutation. Record actual results beside this expectation.
...
Add a contrasting fixture for the main operation: validate a server-issued token and origin for cookie-authenticated mutations. For an industry exercise, write a recovery and handoff note identifying operational limits.
