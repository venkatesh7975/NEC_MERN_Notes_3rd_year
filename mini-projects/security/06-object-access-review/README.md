...
ID: SECURITY-06. Status: specification ready; solution not supplied.
...
Apply owasp basics, sql injection, xss, csrf to implement object access review. Explain the acceptance example from input to result.

## Problem Statement

Build object access review for a fictional college environment. Check ownership for reads and writes; use two fictional accounts.

## Features

- Check ownership for reads and writes.
- use two fictional accounts.

## Functional Requirements

1. FR-1: Check ownership for reads and writes. Demonstrate with a reproducible fixture.
2. FR-2: use two fictional accounts. Demonstrate with a reproducible fixture.
...
Prepare the fixture and interaction needed for this scenario: User A cannot access user B records.

## Expected Output

User A cannot access user B records. Record actual results beside this expectation.
...
Add a contrasting fixture for the main operation: check ownership for reads and writes. For an industry exercise, write a recovery and handoff note identifying operational limits.
