...
ID: AUTHENTICATION-09. Status: specification ready; solution not supplied.
...
Apply passwords, sessions, identity, ownership, roles to implement account deactivation. Explain the acceptance example from input to result.

## Problem Statement

Build account deactivation for a fictional college environment. Reject sessions for disabled accounts; avoid recovery enumeration.

## Features

- Reject sessions for disabled accounts.
- avoid recovery enumeration.

## Functional Requirements

1. FR-1: Reject sessions for disabled accounts. Demonstrate with a reproducible fixture.
2. FR-2: avoid recovery enumeration. Demonstrate with a reproducible fixture.
...
Prepare the fixture and interaction needed for this scenario: Disabled account cannot reuse old session.

## Expected Output

Disabled account cannot reuse old session. Record actual results beside this expectation.
...
Add a contrasting fixture for the main operation: reject sessions for disabled accounts. For an industry exercise, write a recovery and handoff note identifying operational limits.
