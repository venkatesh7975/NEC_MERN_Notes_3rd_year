...
ID: AUTHENTICATION-07. Status: specification ready; solution not supplied.
...
Apply passwords, sessions, identity, ownership, roles to implement password reset simulator. Explain the acceptance example from input to result.

## Problem Statement

Build password reset simulator for a fictional college environment. Store hashed expiring single-use tokens; simulate delivery locally.

## Features

- Store hashed expiring single-use tokens.
- simulate delivery locally.

## Functional Requirements

1. FR-1: Store hashed expiring single-use tokens. Demonstrate with a reproducible fixture.
2. FR-2: simulate delivery locally. Demonstrate with a reproducible fixture.
...
Prepare the fixture and interaction needed for this scenario: Used reset token cannot be replayed.

## Expected Output

Used reset token cannot be replayed. Record actual results beside this expectation.
...
Add a contrasting fixture for the main operation: store hashed expiring single-use tokens. For an industry exercise, write a recovery and handoff note identifying operational limits.
