...
ID: AUTHENTICATION-05. Status: specification ready; solution not supplied.
...
Apply passwords, sessions, identity, ownership, roles to implement owner-only notes. Explain the acceptance example from input to result.

## Problem Statement

Build owner-only notes for a fictional college environment. Scope queries by verified user ID; test other record IDs.

## Features

- Scope queries by verified user ID.
- test other record IDs.

## Functional Requirements

1. FR-1: Scope queries by verified user ID. Demonstrate with a reproducible fixture.
2. FR-2: test other record IDs. Demonstrate with a reproducible fixture.
...
Prepare the fixture and interaction needed for this scenario: Changed URL cannot expose another user note.

## Expected Output

Changed URL cannot expose another user note. Record actual results beside this expectation.
...
Add a contrasting fixture for the main operation: scope queries by verified user id. For an industry exercise, write a recovery and handoff note identifying operational limits.
