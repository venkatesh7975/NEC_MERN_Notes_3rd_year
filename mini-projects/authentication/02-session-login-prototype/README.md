# Session login prototype

ID: AUTHENTICATION-02. Status: specification ready; solution not supplied.
...
Apply passwords, sessions, identity, ownership, roles to implement session login prototype. Explain the acceptance example from input to result.

## Problem Statement

Build session login prototype for a fictional college environment. Create server session; protect route; invalidate on logout.
...
1. FR-1: Create server session. Demonstrate with a reproducible fixture.
2. FR-2: protect route. Demonstrate with a reproducible fixture.
3. FR-3: invalidate on logout. Demonstrate with a reproducible fixture.
...
Prepare the fixture and interaction needed for this scenario: Old session fails after logout.

## Expected Output

Old session fails after logout. Record actual results beside this expectation.
...
Add a contrasting fixture for the main operation: create server session. For an industry exercise, write a recovery and handoff note identifying operational limits.
