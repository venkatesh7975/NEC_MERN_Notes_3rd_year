...
ID: AUTHENTICATION-03. Status: specification ready; solution not supplied.
...
Apply passwords, sessions, identity, ownership, roles to implement token verification sandbox. Explain the acceptance example from input to result.

## Problem Statement

Build token verification sandbox for a fictional college environment. Sign short-lived tokens; verify signature expiry and malformed cases.

## Features

- Sign short-lived tokens.
- verify signature expiry and malformed cases.

## Functional Requirements

1. FR-1: Sign short-lived tokens. Demonstrate with a reproducible fixture.
2. FR-2: verify signature expiry and malformed cases. Demonstrate with a reproducible fixture.
...
Prepare the fixture and interaction needed for this scenario: Expired token is rejected.

## Expected Output

Expired token is rejected. Record actual results beside this expectation.
...
Add a contrasting fixture for the main operation: sign short-lived tokens. For an industry exercise, write a recovery and handoff note identifying operational limits.
