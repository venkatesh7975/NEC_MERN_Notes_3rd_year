# Atomic seat transfer

ID: SQL-08. Status: specification ready; solution not supplied.
...
Apply sql versus nosql, crud, keys, joins to implement atomic seat transfer. Explain the acceptance example from input to result.

## Problem Statement

Build atomic seat transfer for a fictional college environment. Use transaction for two updates; inject failure and roll back.

## Features

- Use transaction for two updates.
- inject failure and roll back.

## Functional Requirements

1. FR-1: Use transaction for two updates. Demonstrate with a reproducible fixture.
2. FR-2: inject failure and roll back. Demonstrate with a reproducible fixture.
...
Prepare the fixture and interaction needed for this scenario: Failed second update preserves both original values.

## Expected Output

Failed second update preserves both original values. Record actual results beside this expectation.
...
Add a contrasting fixture for the main operation: use transaction for two updates. For an industry exercise, write a recovery and handoff note identifying operational limits.
