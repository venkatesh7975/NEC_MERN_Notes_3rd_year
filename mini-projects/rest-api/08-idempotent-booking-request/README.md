...
ID: REST-API-08. Status: specification ready; solution not supplied.
...
Apply resources, methods, status codes, statelessness to implement idempotent booking request. Explain the acceptance example from input to result.

## Problem Statement

Build idempotent booking request for a fictional college environment. Store key and request fingerprint; reject key reuse with different data.

## Features

- Store key and request fingerprint.
- reject key reuse with different data.

## Functional Requirements

1. FR-1: Store key and request fingerprint. Demonstrate with a reproducible fixture.
2. FR-2: reject key reuse with different data. Demonstrate with a reproducible fixture.
...
Prepare the fixture and interaction needed for this scenario: Same key and body returns original booking.

## Expected Output

Same key and body returns original booking. Record actual results beside this expectation.
...
Add a contrasting fixture for the main operation: store key and request fingerprint. For an industry exercise, write a recovery and handoff note identifying operational limits.
