# Conditional update lab

ID: REST-API-07. Status: specification ready; solution not supplied.
...
Apply resources, methods, status codes, statelessness to implement conditional update lab. Explain the acceptance example from input to result.

## Problem Statement

Build conditional update lab for a fictional college environment. Require current revision; reject stale writes with conflict.
...
1. FR-1: Require current revision. Demonstrate with a reproducible fixture.
2. FR-2: reject stale writes with conflict. Demonstrate with a reproducible fixture.
...
Prepare the fixture and interaction needed for this scenario: Two edits at revision 1 allow only the first.

## Expected Output

Two edits at revision 1 allow only the first. Record actual results beside this expectation.
...
Add a contrasting fixture for the main operation: require current revision. For an industry exercise, write a recovery and handoff note identifying operational limits.
