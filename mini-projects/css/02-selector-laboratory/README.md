...
ID: CSS-02. Status: specification ready; solution not supplied.
...
Build readable layouts that adapt to narrow and wide screens. Apply this by building selector laboratory and explaining the acceptance example.

## Problem Statement

Build selector laboratory for a fictional college learning environment. The deliverable must demonstrate the specific behavior below using small reproducible fixtures. Use fictional records only.

## Features

- Style nested lists with element, class and attribute selectors.
- document three competing rules.

## Functional Requirements

1. FR-1: Style nested lists with element, class and attribute selectors. Provide an observation or test that proves this behavior.
2. FR-2: document three competing rules. Provide an observation or test that proves this behavior.
...
Prepare the fixture or interaction in this acceptance scenario: A marked item uses the intended winning selector.

## Expected Output

Acceptance oracle: A marked item uses the intended winning selector. Record the actual result next to the expected result in your test evidence.
...
Add one contrasting fixture that stresses the main invariant: style nested lists with element, class and attribute selectors. Compare the two results in a short design note. For Industry projects, include operational limits and recovery steps in the handoff.

## Interview Questions

1. How does style nested lists with element, class and attribute selectors work in your solution?
