# Multi-page department theme

ID: CSS-10. Status: specification ready; solution not supplied.
...
Build readable layouts that adapt to narrow and wide screens. Apply this by building multi-page department theme and explaining the acceptance example.

## Problem Statement

Build multi-page department theme for a fictional college learning environment. The deliverable must demonstrate the specific behavior below using small reproducible fixtures. Use fictional records only.

## Features

- Apply shared CSS variables across three pages.
- support narrow screens and long unbroken text.

## Functional Requirements

1. FR-1: Apply shared CSS variables across three pages. Provide an observation or test that proves this behavior.
2. FR-2: support narrow screens and long unbroken text. Provide an observation or test that proves this behavior.
...
Prepare the fixture or interaction in this acceptance scenario: Changing one spacing variable updates all pages consistently.

## Expected Output

Acceptance oracle: Changing one spacing variable updates all pages consistently. Record the actual result next to the expected result in your test evidence.
...
Add one contrasting fixture that stresses the main invariant: apply shared css variables across three pages. Compare the two results in a short design note. For Industry projects, include operational limits and recovery steps in the handoff.

## Interview Questions

1. How does apply shared css variables across three pages work in your solution?
