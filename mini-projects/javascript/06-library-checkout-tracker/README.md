...
ID: JAVASCRIPT-06. Status: specification ready; solution not supplied.
...
Predict values, implement branching and loops, and connect logic to browser events. Apply this by building library checkout tracker and explaining the acceptance example.

## Problem Statement

Build library checkout tracker for a fictional college learning environment. The deliverable must demonstrate the specific behavior below using small reproducible fixtures. Use fictional records only.

## Features

- Add and return books by ID.
- prevent a second checkout of the same copy.

## Functional Requirements

1. FR-1: Add and return books by ID. Provide an observation or test that proves this behavior.
2. FR-2: prevent a second checkout of the same copy. Provide an observation or test that proves this behavior.
...
Prepare the fixture or interaction in this acceptance scenario: Checkout B1 twice leaves exactly one active loan.

## Expected Output

Acceptance oracle: Checkout B1 twice leaves exactly one active loan. Record the actual result next to the expected result in your test evidence.
...
Add one contrasting fixture that stresses the main invariant: add and return books by id. Compare the two results in a short design note. For Industry projects, include operational limits and recovery steps in the handoff.

## Interview Questions

1. How does add and return books by id work in your solution?
