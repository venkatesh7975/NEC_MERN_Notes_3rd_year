...
ID: JAVASCRIPT-05. Status: specification ready; solution not supplied.
...
Predict values, implement branching and loops, and connect logic to browser events. Apply this by building marks summary and explaining the acceptance example.

## Problem Statement

Build marks summary for a fictional college learning environment. The deliverable must demonstrate the specific behavior below using small reproducible fixtures. Use fictional records only.

## Features

- Calculate min, max and mean for validated marks 0 to 100.
- preserve the original list.

## Functional Requirements

1. FR-1: Calculate min, max and mean for validated marks 0 to 100. Provide an observation or test that proves this behavior.
2. FR-2: preserve the original list. Provide an observation or test that proves this behavior.
...
Prepare the fixture or interaction in this acceptance scenario: [40,60,80] yields mean 60.

## Expected Output

Acceptance oracle: [40,60,80] yields mean 60. Record the actual result next to the expected result in your test evidence.
...
Add one contrasting fixture that stresses the main invariant: calculate min, max and mean for validated marks 0 to 100. Compare the two results in a short design note. For Industry projects, include operational limits and recovery steps in the handoff.

## Interview Questions

1. How does calculate min, max and mean for validated marks 0 to 100 work in your solution?
