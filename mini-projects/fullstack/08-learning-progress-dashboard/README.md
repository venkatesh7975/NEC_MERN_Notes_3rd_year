...
ID: FULLSTACK-08. Status: specification ready; solution not supplied.
...
Apply integration, persistence, api contracts, verification to implement learning progress dashboard. Explain the acceptance example from input to result.

## Problem Statement

Build learning progress dashboard for a fictional college environment. Persist lesson completion; derive progress from server records.

## Features

- Persist lesson completion.
- derive progress from server records.

## Functional Requirements

1. FR-1: Persist lesson completion. Demonstrate with a reproducible fixture.
2. FR-2: derive progress from server records. Demonstrate with a reproducible fixture.
...
Prepare the fixture and interaction needed for this scenario: Repeated completion does not double count.

## Expected Output

Repeated completion does not double count. Record actual results beside this expectation.
...
Add a contrasting fixture for the main operation: persist lesson completion. For an industry exercise, write a recovery and handoff note identifying operational limits.
